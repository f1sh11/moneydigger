...
def fetch_all_cases(keywords=("武器箱", "纪念包", "收藏品", "收藏包"), cookies=None):
    all_cases = []
    seen_ids = set()
    ...
    for page in range(1, MAX_PAGE_LIMIT + 1):
        url = (
            ...
            f"?game=csgo&page_num={page}&use_suggestion=0"
        )
...
def save_to_json(cases, filename="buff_all_case_types.json"):
...
if __name__ == "__main__":
    ...
    all_cases = fetch_all_cases(cookies=cookies)
    print(f"\n🎯 共找到所有箱子类型（含纪念包/收藏品/武器箱）：{len(all_cases)} 个\n")
